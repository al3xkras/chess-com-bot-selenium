...
if __name__ == '__main__':
    ...
    def get_last_move(driver: webdriver.Chrome):
        ...
        # piece_type = [x for x in second.get_attribute("class").split() if len(x)==2][0]
    ...
    def actions(driver: webdriver.Chrome, session):
        ...
        while loop < loops_count_max:
            ...
            wait_time = op_time + abs(np.random.random()) * time_std + time_avg
            sleep(0.1)
            t1, t2 = get_last_move(driver)
            last_move = t2
            move_ = t1 + t2
